...
class PlaceEmoji():
    def __init__(self, faces_recognition_classifier=None):
        if(faces_recognition_classifier is None):
            self.faces_recognition_classifier = cv.CascadeClassifier(cv.data.haarcascades + 'haarcascade_frontalface_default.xml') # Model pour reconnaître plusieurs viages.
        else:
    ...
    def __Get_faces_data(self, img, with_best_emotion_model):
        ...
        for (x, y, w, h) in rectangle_detected:
            ...
            if (mood == None): # Ignore les coordonnées qui ne sont pas un visage dans le cas où ce n'est ni un visage masqué, ni un visage avec des émotions.
    ...
    def __Place_emoji(self, img):
        ...
        for face_data in self.__faces_data:
            ...
            img[faces_y2_point : faces_y1_point, faces_x2_point : faces_x1_point] = get_emoji(face_data['mood'], (width, height)) # Récupère l'émoji correcpondante au visage et le remplace dans l'image.
    ...
    def Get_image_with_faces(self, image, with_best_emotion_model = True):
        ...
        img_with_rectangle = copy.deepcopy(image) # Copie l'image pour pouvoir faire toutes les opérations sans affecter celle-ci.
        self.__Get_faces_data(img_with_rectangle, with_best_emotion_model) # Récupère et stocke dans une liste l'ensemble des info lié aux visages trouvés.
        self.__Place_emoji(image) # Place les émoji sur l'image fourni grâce aux informations receuillies par al fonction au dessus.
        return image
